...
import OwnerViewAllReviews from "./OwnerViewAllReviews";
import userEvent from "@testing-library/user-event";


describe('Owner View All Reviews', () => {

    test('should render the header', () => {
        render(<Router><OwnerViewAllReviews /></Router>);
        const h1Elem = screen.getByRole('heading', { level: 1, name: /no reviews/i });
        expect(h1Elem).toBeInTheDocument();
    })

    test('should render navbar correctly as well', () => {
        const view = render(<Router><OwnerViewAllReviews></OwnerViewAllReviews></Router>)
