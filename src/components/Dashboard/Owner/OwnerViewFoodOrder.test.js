...
import OwnerViewFoodOrder from "./OwnerViewFoodOrder";
import userEvent from "@testing-library/user-event";


describe('Owner View Food Order', () => {

    test('should render the header', () => {
        render(<Router><OwnerViewFoodOrder /></Router>);
        const h1Elem = screen.getByRole('heading', { level: 1, name: /no food/i });
        expect(h1Elem).toBeInTheDocument();
    })

    test('should render navbar correctly as well', () => {
        const view = render(<Router><OwnerViewFoodOrder></OwnerViewFoodOrder></Router>)
